...
class Controller:
    ...
    def roll(self, message):
        ...
        if len(msg) == 1:
            ...
            return messages.roll(message.author, result)

        elif len(msg) == 2:
            if isinstance(msg[1], int):
                result = actions.dice(msg[0])
                return messages.roll(message.author, result, msg[1])
            else:
                result = actions.dice(msg[0], advantage=msg[1])
                return messages.roll(message.author, result, advantage=1)

        elif len(msg) == 3:
            try:
                result = actions.dice(msg[0], advantage=msg[2])
                return messages.roll(message.author, result, msg[1], advantage=1)
            except:
